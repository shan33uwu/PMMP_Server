...
class GuildCommand extends BaseCommand
{
    ...
    {
        parent::__construct('guild', $plugin);

        $this->setPermission(Permissions::DEFAULT_COMMAND_PERMISSION);
        $this->setAliases(['g', 'guilds']);
        $this->setDescription('Command used for displaying guild GUI');
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        if ($sender instanceof Player) {
            ...
            if ($plugin->getServerManager()->enableSocialManager()) {
                $plugin->getPlayerManager()->getSocialManager()->getGuildsManager()->sendGuildMenu($sender);
            } else {
