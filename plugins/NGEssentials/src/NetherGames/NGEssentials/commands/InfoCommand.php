...
class InfoCommand extends BaseCommand
{
    ...
    public function __construct(NGEssentials $plugin)
    {
        ...
        $this->setAliases(['i']);
        $this->setDescription('View player-related information');
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        ...
        if (isset($args[0])) {
            ...
            $enforcementHandler->getPortalInfo($playerManager->getBestMatchingPlayer($args[0]), static function ($portalInfo) use ($sender, $enforcementHandler): void {
                if ($sender->isConnected()) {
                    if ($portalInfo instanceof StaffPortalInfo) {
                        $enforcementHandler->sendInfo($sender, $portalInfo);
