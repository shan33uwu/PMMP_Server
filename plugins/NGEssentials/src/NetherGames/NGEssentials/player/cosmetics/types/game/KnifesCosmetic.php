...
class KnifesCosmetic extends Cosmetic
{
    use ItemCosmeticTrait;

    public function get(Player $player): ?Item
    {
        return ($entry = $this->getSelectedEntry($player)) === null ? null : $this->getItem($entry->getDataEntry());
    }

    public function getCrateAnimation(): string
    {
        return 'animation.ng.lobby.crate.knife';
    }
    ...
    {
        return new ImageButton(SimpleForm::BUTTON_TAB . $this->getName(), ImageButton::IMAGE_TYPE_PATH, 'textures/items/iron_sword', $callable);
    }
}