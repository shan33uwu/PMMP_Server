...
class HoveringEntityNavigator extends LazyPathFindingNavigator
{
    public function __construct(EntityBase $holder)
    {
        parent::__construct(
            $holder,
            new EntityNavigator($holder),
            new PathFindingNavigator(
                $holder,
                new AStarPathFindingAlgorithm(
                    DistanceFunction::euclideanSquared(...),
                    DistanceFunction::manhattan(...),
                    new HoveringNeighborResolver($holder->getBoundingBox()),
                    16
    ...
    {
        return $this->holder->getSize()->getWidth() / 2;
    }

    protected function shouldUsePathfinding(): bool
    {
        return $this->holder->isCollided;
    }
}