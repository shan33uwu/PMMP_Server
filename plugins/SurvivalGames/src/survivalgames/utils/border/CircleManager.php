...
namespace survivalgames\utils\border;
...
/**
 * Border class for SurvivalGames, contains shrinking, expanding, moving mechanics
 * for a border to operate correctly using the basics of trigonometry.
 *
 * <p>All mechanics has been tested carefully and no major bugs were present.
 *
 * @author larryTheCoder
 * @package survivalgames\utils\border
 */
class CircleManager
{
    ...
    public function __construct(Vector3 $midPoint, array $config = [])
    {
    ...
    public function renderParticles(Player $player): void
    {
        ...
        if (rad2deg($refAngle) >= 45) {
            ...
            for ($x = $normX - 5; $x <= ($normX + 5); $x++) {
                $refAngle2 = acos(max(-1.0, min(1.0, $x / $length)));
            ...
            for ($z = $normZ - 5; $z <= ($normZ + 5); $z++) {
                $refAngle2 = asin(max(-1.0, min(1.0, $z / $length)));
        ...
        foreach ($positions as $pos) {
            for ($y = -1; $y <= 7; $y++) {
                $player->getWorld()->addParticle($pos->add($midPoint->getFloorX(), $y, $midPoint->getFloorY()), new RedstoneParticle(), [$player]);
            }
        }
    }
