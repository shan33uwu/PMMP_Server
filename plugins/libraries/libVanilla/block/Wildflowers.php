...
class Wildflowers extends Flowable implements HorizontalFacing
{
    ...
    {
        return ($blockReplace instanceof Wildflowers && $blockReplace->count < self::MAX_COUNT)
            || $this->supportedWhenPlacedAt($blockReplace, $clickVector, $face, $isClickedBlock);
    }

    public function place(BlockTransaction $tx, Item $item, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, ?Player $player = null): bool
    {
        if ($blockReplace instanceof Wildflowers && $blockReplace->count < self::MAX_COUNT) {
