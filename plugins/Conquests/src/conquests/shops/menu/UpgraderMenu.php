...
use conquests\shops\UpgradeTier;
...
final class UpgraderMenu
{
    ...
    public const MENU_IDENTIFIER = "conquests:upgrader";
    ...
    public function send(Player $player, CQTeam $team): void
    {
    ...
    public function handleMenuTransaction(DeterministicInvMenuTransaction $transaction): void
    {
        ...
        if (!($arena = Conquests::getInstance()->getArena($player)) instanceof CQArena) {
        ...
        match (true) {
            ...
            default => Conquests::getInstance()->getLogger()->warning("Unhandled upgrader menu transaction for player {$player->getName()}")
        };
    }

    public function handleUpgradeTransaction(DeterministicInvMenuTransaction $transaction, CQTeam $team): void
    {
        /** @var NGPlayer $player */
        $player = $transaction->getPlayer();
        $itemClicked = $transaction->getItemClicked();
    ...
    public function handleTrapMenuTransaction(DeterministicInvMenuTransaction $transaction, CQTeam $team): void
    {
        /** @var NGPlayer $player */
        $player = $transaction->getPlayer();
        $itemClicked = $transaction->getItemClicked();
    ...
    public function sendTrapMenu(InvMenuInventory $inventory, Player $player, CQTeam $team): void
    {
    ...
    public function sendUpgradesMenu(InvMenuInventory $inventory, Player $player, CQTeam $team): void
    {
        $inventory->clearAll();
        $index = self::STARTING_INDEX;
